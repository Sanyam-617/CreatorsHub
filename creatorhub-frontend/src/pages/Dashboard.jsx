...
// BUG FIX: removed unused `useAuth` import — `user` was destructured but
// never used in this component.
...
const Dashboard = () => {
  ...
  // BUG FIX: the original JSX had a broken structure — the closing tags for
  // .page-header-left and .page-header were mismatched (the live-indicator was
  // placed outside .page-header-left but before the closing .page-header div,
  // and then all the content sections were indented as if still inside
  // .page-header, producing invalid nesting). Corrected below.
  return (
    <PageWrapper>
      <div className="main-content">
        ...
        {/* Empty State */}
        {!loading && !error && (!data || !data.combined) && (
              ...
              >
                {platforms.map((p) => (
                  <Area
                    ...
                    stroke={
                      p.platformName === 'youtube' ? '#ff4444'
                      : p.platformName === 'instagram' ? '#cc66ff'
                      : '#4da6ff'
                    }
                    strokeWidth={2}
                    fill={
                      p.platformName === 'youtube' ? 'url(#gradient-ff4444)'
                      : p.platformName === 'instagram' ? 'url(#gradient-cc66ff)'
                      : 'url(#gradient-4da6ff)'
                    }
                    dot={false}
                    activeDot={{
                      r: 5,
                      fill: p.platformName === 'youtube' ? '#ff4444'
                        : p.platformName === 'instagram' ? '#cc66ff'
                        : '#4da6ff',
                      stroke: '#141414',
                      strokeWidth: 2,
                    ...
                    <div className="platform-header">
                      ...
                      <div className="platform-info">
                        <div
                          ...
                          style={{
                            color: p.platformName === 'youtube' ? '#ff4444'
                              : p.platformName === 'instagram' ? '#cc66ff'
                              : '#4da6ff',
                          }}
                        >
